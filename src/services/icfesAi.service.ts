...
import type { OpcionRow } from './icfesTypes';
import { z } from 'zod';
...
const generatedQuestionsSchema = z.object({
  preguntas: z.array(z.object({
    textoPregunta: z.string().min(1),
    nombreCompetencia: z.string().min(1),
    peso: z.number().finite(),
    explicacionRespuesta: z.string().optional(),
...
const parseJsonFromContent = (raw: string): unknown => {
  ...
  if (genericBlock?.[1]) return JSON.parse(genericBlock[1]);
  throw new Error('No fue posible interpretar la respuesta JSON del modelo.');
};

export const generateQuestionsWithOpenAI = async (
  payload: { tema: string; textoBase: string; dificultad: string; cantidad: number },
  competencias: Array<{ nombre: string; peso: number }>
) => {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error('OPENAI_API_KEY no esta configurada en el backend.');
...
FORMATO ESTRICTO DE SALIDA JSON (respeta exactamente estas llaves):
{
  ...
  if (!content || typeof content !== 'string') throw new Error('OpenAI no devolvio contenido util.');

  const parsed = generatedQuestionsSchema.parse(parseJsonFromContent(content));
  return { preguntas: parsed.preguntas, systemPrompt };
};

export const generarExplicacionErrorIa = async (intentoId: number, preguntaId: number, matriculaNo: number) => {
  ...
  if (cacheRs.recordset.length) return { explicacion: cacheRs.recordset[0].Explicacion, cached: true };

  const detalleRs = await pool.request()
    .input('intentoId', sql.Int, intentoId)
    .input('preguntaId', sql.Int, preguntaId)
    .input('matriculaNo', sql.Int, matriculaNo)
    .query<{ TextoPregunta: string; ExplicacionRespuesta: string | null; OpcionIDSeleccionada: number | null; EsCorrecta: boolean }>(`
  ...
  if (!detalleRs.recordset.length) throw new Error('No se encontro informacion de la pregunta en este intento.');
  const detalle = detalleRs.recordset[0];
  if (detalle.EsCorrecta) return { explicacion: 'Esta respuesta fue correcta. Buen trabajo.', cached: false };

  const opcionesRs = await pool.request().input('preguntaId', sql.Int, preguntaId).query<OpcionRow>(`
...
Responde en maximo 120 palabras con este formato: 1) Error principal cometido. 2) Por que la correcta si responde lo pedido. 3) Tip practico para no caer en esa trampa de nuevo.`;
  ...
  const aiRs = await axios.post(
    OPENAI_URL,
    { model: OPENAI_MODEL, temperature: 0.5, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }] },
  ...
  if (!explicacion) throw new Error('No fue posible generar explicacion con IA.');

  await pool.request()
    .input('intentoId', sql.Int, intentoId)
    .input('preguntaId', sql.Int, preguntaId)
    .input('explicacion', sql.NVarChar(sql.MAX), explicacion)
    .input('modelo', sql.NVarChar(100), OPENAI_MODEL)
    .query(`
